...
#   fi
#   if [ "$?" -ne 0 ]; then
#     printf "ERROR: Failed during \'make -j"$(nproc)" $1\' in $(pwd)\n"
#     exit 1
#   fi
# } 
...
if [[ $ROOTFS = 1 ]]; then
  ...
  printf "* Export NFS share:\n\t> sudo exportfs -o rw,no_subtree_check,fsid=root,anonuid=$(id -u),anongid=$(id -u) ${BOARDIP}:${WD}/nfs/\n"
fi
